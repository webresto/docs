...
> <span style="background:red;color:white;border-radius:5px;padding:3px">🚧 deprecated</span> Будет вытеснен компонентом с inverntory, c возможностью рекурсивно вкладывть компоненты друг в друга. Для root компонента будет введен спецальный флаг который будет иметь аналогичную с layout логику
...
**[α]** Сам компонент может содержать инвентори
