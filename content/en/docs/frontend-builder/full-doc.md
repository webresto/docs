...
**[α]** - этим значком помечен функцонал который находится в стадии альфа-версии

## Термины

**Рецепт** или (*конфиг сборки*) - Указания в виде JSON по которому генерируется фронтенд *(пример дефолтного конфига должен лежать в корне репозитория файл: **recipe.json**)*

---

**Библиотека компонентов** она же (*Фабрика*) - Общая папка со всеми вариациями компонентов. Описанными манифестами. Из этого репозитория билдер строит фронтенд. Билдер имеет поддержку нескольких репозиториев. Тоесть в рецепте мы можем указать из какого репозитория будет собран фронтенд. 
...
**Инвентори/inventory** - Любой компонент может содержать inventory, ключ который находится в свойстве инвентори это одновременно group для поиска, и папка в которую будет доставлен компонент билдером
...
**Манифест** - Файл `index.m.yml`\`manifest.yml` хранится в корне проекта, содержит технические константы, описания экшенов для Юнитов, для каждого компонента должен быть написан также свой файл манифеста. `$component_name$.m.yml` и находится в корне папки компонента. Посути манифест является типизацией состояний, и свойст компонента. Билдер будет читать этот файл для того чтобы построить структуру компонентов и их свойств которые ему доступны.

**Группа компонентов/Group** - Группа обьеденяет разные компоненты по их назначению, для каждого компонента в inventory указывается группа, но она также может не указыватся если указан ключ для компонента. Также инвентори может быть готов забрать любую их нескольких груп, тогда свойство inventory[$key].groups будет массивом. Но при этом в финальный проект key будет являтся именем папки для компонента.

> **[α]** - имя файла манифеста будет приведено к общему виду `manifest.yml`

Два типа манифестов
1. Манифест проекта (index.m.yml)
2. Манифест юнита
    1. Layout - описание верстки с возможностью добавлять дочерние компоненты
    2. Component - единица строения билдера, может содержать kit для того чтобы получать дополнительный функционал
    3. Icon pack - Набор иконок
    4. Kit - Набор компонентов
...
UNIT: **Component**  -  Наполняют шаблон, компоненты могут переходить между шаблонами (т.е. между проектами). У компоненты может быть 5 настраиваемых свойств (цвета, шрифты, состояния, переменные, темы).
...
**[α]** *Ассеты компонента* - папка с ресурсами которая располагается внутри компонента, после обработки билдером переносится по пути взятого из корневого манифеста.

_пример манифеста для компонента_

**[α]** Сам компонент может содержать инвентори, инвентори может быть обязательным и не обязательным. В случае не обязательной установки должно подразумеватся что может прийти пустое значение. Рекомендуется это покрывать тестом!

Компонент может содержать kit через inventory

флаг isRoot: true включает режим layout для компонета
...
UNIT: **Набор (kit)** - **[α]** Набор компонентов может содержать любое колличество других компонентов, которые могут быть использованы для построение фронтенда. Набор может быть назначен в инвентори лейоута подобно любому компоненту. 
Также kit может быть применен для компонента через inventory. 
Для того чтобы компоненты могли быть загружены в приложение, билдер генерирует массив который содержит все предоставленные компоненты. 

Шаблон для генератора кода импорта kit может быть переопределен если создать файл kit.tmpl [синтакисис](https://github.com/Shopify/liquid/wiki/Liquid-for-Designers) то будет создан файл по этому шаблону, для того чтобы изменить имя файла или путь до него нужно переопределить `component.kit.template` 
...
> * `item` читается из файла package.json компонента или может быть передан при конфигурации в рецепте (приоритетнее)

Чтобы загрузить компонент в `kit` в рецепте указывается: 
...
далее настройка происходит аналогично тому как настраивается обычный компонент. 
см. примеры ниже (ссылка)[]:
